...
::: info
可以通过复制粘贴图片下方的JSON代码到程序画布，快速测试节点效果。
:::

## 节点执行次序

每个程序有且只有一个`控制流/程序结束`节点，可以通过画布右上角![Location](quick-start/location.webp)按钮快速定位到该节点。除了`控制流/分支`节点和`控制流/循环`类节点，其它节点皆从左到右，从上到下依次执行。
...
除了连接至`循环体`的节点，其它节点只会执行一次（即使同个输出端连接到多个不同的节点）。
