...
程序（在这里也可以称之为“脚本”）是我们要编写的运行逻辑。

新建程序只需点击程序列表面板的“+”号按钮，打开新建程序对话框，填写程序名称和描述后再点击“确认”即可：

![](quick-start/PixPin_2026-07-01_10-30-45.webp)

![](quick-start/PixPin_2026-07-01_10-39-37.webp)

以上便是我们刚刚创建的第一个程序，按快捷键<kbd>F5</kbd>或点击![](quick-start/PixPin_2026-07-01_10-43-17.webp)按钮即可运行（快试一试吧😊）。

程序由节点组成，而节点又由“输入端”（或称“输入连接器”）和“输出端”（或称“输出连接器”）组成；节点间通过连接它们的输入端和输出端来决定数据传输方向和执行次序：
...
点击选项面板的“+”号按钮打开新建选项对话框，配置用途，类型和名称，最后点击“确认”按钮即可完成选项的创建：
...
执行器可以为程序配置不同的选项值，以此处理不同的任务，多个执行器可以并行运行（需订阅增强版）。
