...
以下规则适用于所有节点，各节点文档中不再重复说明。

- **求值时机**：节点仅在输出连接器被连接时才执行计算；多输出节点对每个输出单独计算，未连接的输出不会被计算，也不产生副作用。
- **输入默认值**：未连接的输入连接器使用其默认值参与运算。
- **流程控制信号**：流程控制（➡️）连接器用于驱动执行流；节点执行完成后向已连接的输出发出流程信号，后续节点随之执行。
- **变长节点**：连接器数量可增减的节点，可通过界面 +/− 按钮或 CLI `program node connector add/delete` 调整（每次增减一组连接器）。
- **格式驱动节点**：连接器数量由格式字符串中的 `{}` 占位符数量决定，修改格式值会自动同步参数连接器；`connector set` 的响应中会报告因此新增/删除的连接器。
- **只读连接器**：被标记为只读的连接器（如格式字符串输入）不可与其它连接器连接，即运行时只读。
